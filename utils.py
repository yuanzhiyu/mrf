...
def load_data(dataset, frac_num=1.0):
    # Load the data
    data = np.array(pd.read_csv(f'data/{dataset}/{dataset}.data', sep=',', header=None, skiprows=0).sample(frac=frac_num))
    features_attr = list(pd.read_csv(f'data/{dataset}/type.data', sep=',', header=None, skiprows=0).values.flatten())
